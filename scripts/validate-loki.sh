...
#
# Purpose:
#  - Verify that Loki is installed and running correctly
#  - Confirm the HTTP API is reachable and healthy
#  - Optionally perform a real functional round-trip test
...
#  - Use infra-bash-lib helpers for output, service checks, and file/system checks
#
# Notes:
#  - This script validates Loki only
#  - It does not validate Promtail, Grafana, or the full logging pipeline
#  - Readiness mode is sufficient for stage validation
#  - Functional mode is recommended for stronger proof of correctness
#
...
#
#  - LOKI_QUERY_RETRIES
#      Number of query retry attempts in functional mode
#      Default: 10
#
#  - LOKI_QUERY_SLEEP_SECONDS
#      Delay between query attempts in functional mode
#      Default: 2
#
# Usage:
...
query_for_validation_log() {
	...
	encoded_query="$(python3 - "$VALIDATION_STREAM_JOB" "$VALIDATION_STREAM_SOURCE" <<'PY'
import sys
import urllib.parse

job = sys.argv[1]
source = sys.argv[2]
query = f'{{job="{job}",source="{source}"}}'
